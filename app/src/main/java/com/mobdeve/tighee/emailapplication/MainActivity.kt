...
class MainActivity : AppCompatActivity() {
    ...
    private val newEmailResultLauncher = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK) {
            ...
            // 1. Extract new email information from intent coming from NewEmailActivity,
            val receiver: String = result.data?.getStringExtra(NewEmailActivity.NEW_RECEIVER_KEY)!!
            val subject: String = result.data?.getStringExtra(NewEmailActivity.NEW_SUBJECT_KEY)!!
            val body: String = result.data?.getStringExtra(NewEmailActivity.NEW_BODY_KEY)!!

            // 2. Create a new Email
            val email = Email(receiver, subject, body)

            // 3 Add Email object to data
            MainActivity.data.add(0, email) // Add to the top of the list
